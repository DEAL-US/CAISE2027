/**
 * Sourced from https://github.com/jellyfin/jellyfin-vue
 */
...
const common = () => defineConfig([
  ...
  },
  {
    ...
    name: '(@caise2027-website/configs/lint/typescript-vue - typescript-eslint) Extended config from plugin (stylistic type checking)'
  },
  {
    ...tseslint.configs.eslintRecommended,
    name: '(@caise2027-website/configs/lint/typescript-vue - typescript-eslint) Extended config from plugin (ESLint rules with type checking)'
  },
  {
    ...regexp.configs[recommendedKey],
    name: '(@caise2027-website/configs/lint/typescript-vue - regexp) Extended config from plugin'
  },
  {
    ...promise.configs[recommendedKey],
    name: '(@caise2027-website/configs/lint/typescript-vue - promise) Extended config from plugin'
  },
  {
    name: '(@caise2027-website/configs/lint/typescript-vue - promise) Custom config',
    rules: {
  ...
  {
    name: '(@caise2027-website/configs/lint/typescript-vue - import) Custom config',
    plugins: {
  ...
  {
    name: '(@caise2027-website/configs/lint/typescript-vue - JSDoc) Custom config',
    plugins: {
      jsdoc
    },
    rules: {
   ...
   */
  {
    name: '(@caise2027-website/configs/lint/typescript-vue - TypeScript & Vue) Disable no-unsafe-* rules',
    rules: {
  ...
  {
    name: '(@caise2027-website/configs/lint/typescript-vue - TypeScript & Vue) Custom config',
    rules: {
...
const vue_config = defineConfig([
  ...vue.configs[recommendedKey].map((config) => {
    ...
    config.name = `(@caise2027-website/configs/lint/typescript-vue - ${config.name}) - Extended config from plugin`;

    return config;
  }),
  {
    ...flatArrayOfObjects(vueScopedCSS.configs[recommendedKey]),
    name: '(@caise2027-website/configs/lint/typescript-vue - Vue Scoped CSS) Extended config from plugin',
    files: vueFiles
  },
  {
    ...css.configs[recommendedKey],
    name: '(@caise2027-website/configs/lint/typescript-vue - Vue CSS) Extended config from plugin',
    files: vueFiles
  },
  {
...
export function getTSVueConfig(enableVue = true, tsconfigRootDir = import.meta.dirname) {
  ...
  const base_vue_parser = {
    name: '(@caise2027-website/configs/lint/typescript-vue - Vue) Extra parser configuration for typed linting',
    files: vueFiles,
    languageOptions: {
      ...langOptions,
      parser: vueParser,
  ...
  const base_ts_parser = {
    name: enableVue
      ? '(@caise2027-website/configs/lint/typescript-vue - TypeScript & Vue) Parser extra config'
      : '(@caise2027-website/configs/lint/typescript-vue - TypeScript) Parser extra config',
